...
public:
	...
	Date();																//デフォルトコントラクタ
	Date(int yy, int mm = 1, int dd = 1);	//コントラクタ
	bool leap_year() const { return leap_year(y); }	// ?
	int year()	const { return y; }		// 年を返却 なんだったけ？
	...
	void set_day(	 int dd)	{ d = dd; }	// 月をmmに設定
...
std::ostream& operator<<(std::ostream& s, const Date& x);	//挿入子供
std::istream& operator>>(std::ostream& s,		Date& x);	//抽出子
